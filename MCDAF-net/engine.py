...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # #加载主干模型的参数
        # checkpoint = torch.load(r'D:\pytest\pycode\detr-main\detr-r50-e632da11.pth')  # 加载预训练模型的state_dict
        # filtered_dict = {k: v for k, v in checkpoint.items() if k.startswith('backbone')}
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    if coco_evaluator is not None:
        ...
        # with open("picture/PR-result.json", "a") as f:  # 使用"a"模式以追加内容方式打开文件
